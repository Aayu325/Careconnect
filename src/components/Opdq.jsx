...
const Opdq = () => {
  const [patients, setPatients] = useState(() => {
    ...
    // Reset if stored data is from a different day
    if (storedData.date !== today) {
      return [];
    }
    return storedData.patients || initialPatients;
  });
  ...
  useEffect(() => {
    const today = new Date().toDateString();
    localStorage.setItem('dailyPatients', JSON.stringify({ date: today, patients }));
  }, [patients]);
  ...
  return (
    ...
    <div className="p-6 mt-10 mb-10 ml-20 md:ml-60 md:mr-10 rounded-2xl bg-indigo-100 min-h-screen">
      <h2 className="text-3xl font-bold mb-6 text-gray-800">Daily Patient Management - OPDQ</h2>
